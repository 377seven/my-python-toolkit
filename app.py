...
# ========== 硅基流动 AI 配置 ==========
# 注意：请替换成你自己的 API Key（从 https://cloud.siliconflow.cn 获取）
SILICONFLOW_API_KEY = st.secrets.get("siliconflow_api_key", "你的默认Key（可选）")
SILICONFLOW_URL = "https://api.siliconflow.cn/v1/chat/completions"

def ai_reply(prompt):
    """调用硅基流动大模型（默认使用 DeepSeek-V3）"""
    if not SILICONFLOW_API_KEY or SILICONFLOW_API_KEY == "你的默认Key（可选）":
        return "❌ 请先在 Streamlit Cloud 的 Secrets 中配置 siliconflow_api_key"
    headers = {
        "Authorization": f"Bearer {SILICONFLOW_API_KEY}",
        "Content-Type": "application/json"
    }
    payload = {
        "model": "deepseek-ai/DeepSeek-V3",  # 可换成 Qwen/Qwen2.5-7B-Instruct
        "messages": [{"role": "user", "content": prompt}],
        "temperature": 0.7,
        "max_tokens": 1024
    }
    try:
        res = requests.post(SILICONFLOW_URL, json=payload, headers=headers, timeout=30)
...
if menu == "首页":
        ...
        try:
            ...
            if st.button("AI一键生成报告"):
                with st.spinner("AI正在生成报告..."):
                    ...
                    st.markdown(report)
                    st.download_button("下载报告TXT文件", report, file_name=f"生产分析报告_{datetime.now().strftime('%Y%m%d%H%M%S')}.txt")
        except Exception as e:
            st.error(f"文件处理出错: {e}")
        ...
        try:
            ...
            if st.button("生成周报"):
                with st.spinner("AI正在写周报..."):
                    ...
                    st.markdown(weekly)
                    st.download_button("下载周报TXT文件", weekly, file_name=f"生产周报_{datetime.now().strftime('%Y%m%d%H%M%S')}.txt")
        except Exception as e:
            st.error(f"文件处理出错: {e}")
    ...
    if uploaded_files:
        ...
        try:
            ...
            merger.close()
            buffer.seek(0)
            st.download_button("下载合并后的PDF", buffer, file_name=f"合并PDF_{datetime.now().strftime('%Y%m%d%H%M%S')}.pdf")
        except Exception as e:
            st.error(f"PDF合并失败: {e}")

elif menu == "数据库存查询(SQL)":
    st.subheader("📊 SQL员工数据库管理（面试演示版）")
    st.info("支持Excel一键入库、自动去重、高频统计查询")
    ...
    with col1:
        if st.button("一键清洗重复数据"):
            cursor = conn.cursor()
            cursor.execute('DELETE FROM staff WHERE id NOT IN (SELECT MIN(id) FROM staff GROUP BY name, email)')
            conn.commit()
    ...
    if search_key:
        res = pd.read_sql("SELECT * FROM staff WHERE name LIKE ? OR dept LIKE ?", conn, params=(f"%{search_key}%", f"%{search_key}%"))
        st.dataframe(res)
